...
<a href="boardList.do?page=${page}"><button type=button class="btn btn-primary">목록</button></a>
<c:choose>
	<c:when test="${!empty sessionScope.logid && sessionScope.logid == board.writer}">
	<a href="removeForm.do?boardNo=${board.boardNo}&page=${page}"><button type=button class="btn btn-danger">삭제 화면</button></a>
	<a href="modifyForm.do?boardNo=${board.boardNo}&page=${page}"><button type="button" class="btn btn-info">수정 화면</button></a>
	</c:when>
	<c:otherwise>
